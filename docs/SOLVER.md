...
### 6.5-4. C2-2: 精緻化と知見(2026-06-13)
**実装**: ①3bet/4bet サイズを Phase B の pot に整合(3bet 11 / 4bet 24 → srp 5.5 / 3bet 22.5 が一致)。②HU seen-flop 終端を **Phase B の解いたサブゲーム V 行列**で評価(`huSeenFlopEV` 解決器・UTG/MP/CO/BTN SRP + BTN/CO 3bet・support<0.5 は flat フォールバック・未被覆対は flat)。

| 席 | C2-2 open%(Phase B V) | flat のみ | アンカー |
|----|----------------------|----------|---------|
| UTG | **15.5** ✓ | 15.4 | 13-18 |
| MP | **18.3** ✓ | 18.4 | 15-22 |
| CO | **24.1** ✓ | 24.2 | 22-30 |
...
安定性 Δ(300↔600 反復)≤0.6。手ごと戦略も妥当(BTN: AA-AKs/AJo/55=1.0・98s=0.50 ミックス・72o≈0)。

**知見(重要)**: **Phase B V 行列の配線は flat 実現率と差 ≤0.4pt** = 較正済 flat(IP/OOP 実現率)が解いたサブゲーム EV をよく近似し、**オープン幅は seen-flop EV の精度に頑健**。→ 精緻化の余地は seen-flop EV ではなく抽象自体(no-limp 等)にある。V 配線は provenance/原理整合のため保持。

**SB の構造的限界**: 29.1(vs 35-58)= **no-limp 抽象**(raise-or-fold は GTO の SB リンプ多用を表現できない)+ OOP 実現率。`srp-sb-bb` は open 2.5≠モデルの 3.0 / IP-OOP ラベルの緊張で**未配線**(ミスマッチモデルで無理にアンカーへ寄せない = 正直表示)。SB を埋めるにはリンプ抽象(木拡張=別タスク)。

### 6.5-5. 残課題・採用判断
- **5bet-allin(MAX_RAISE=4)完全木の検証 ✅**: 29,105 の allin 終端(N-way 厳密)を実求解で初通過(200 反復・8 分・NaN/破綻なし・info-set 5.74M)。結果は 4bet-cap と頑健に同傾向(UTG13.5/MP16.8/CO21.9/BTN39.6/SB28.1 = 5bet 追加で全席 ~2pt 締まる・UTG は手作り 13.4 とほぼ一致)。**結論: 結果は betting-tree 深さに頑健**。本量産(高反復・候補レンジ確定)は一晩計算。
- 残: **リンプ抽象**(SB/BvB の本丸・木拡張 = 別タスク)。
- 採用ゲート **C-2a**(解 JSON 配給のみ・表示レンジ不変)/ **C-2b**(フル置換)。候補レンジ(`scripts/out/preflop-multiway/`・gitignore・src/ 未採用)は UTG/MP/CO/BTN でアンカー妥当。**中止基準: SB のようにアンカー大逸脱が抽象で残るスポットは `solver_model` 出荷せず据え置き**。ROI 注記: 手作りレンジは既に現代 GTO 理論と突合済(R11/R19)→ 本ビルドの主利得は ①Phase C 構造的誤りの解消 ②solver 由来 provenance。配給は product/正直表示の判断 = 明示承認後。

求解器: `preflopMultiwayGame.ts`(木 + CFR+ + 終端 EV + `huSeenFlopEV`) / `preflopEquity.ts`(`nWayEquity` 追加) / `scripts/solve-preflop-multiway.ts`(全席求解 + Phase B V 解決器 + アンカー差分)。

---

## 6.6. Phase V1 — per-hand 検証(2026-06-13)= solver-grade ロードマップ起点

C2 候補レンジ・手作りレンジを**公開 GTO 6-max RFI**(`scripts/published-ranges.ts`・公開チャートの手リストを自前書き起こし = L1 順守)と**セル単位**で突合(`scripts/validate-preflop.ts`)。計画: `~/.claude/plans/preflop-solver-grade-roadmap.md`。

| 席 | 公開幅 | C2 in-out 一致% | 手作り 一致% | C2 L1 距離% |
|----|-------|----------------|-------------|------------|
| UTG | 17% | **94.6**(手作り超) | 92.5 | 6.3 |
| MP | 21% | 91.6 | **94.7** | 8.5 |
| CO | 28% | 89.7 | **93.5** | 10.0 |
| BTN | 43% | 85.7 | **91.1** | 14.6 |

- **C2 は per-hand でも公開 GTO に近い**(86–95% 一致 = ランダムでない実シグナル)。UTG は手作りを上回る。
- ただし **MP/CO/BTN は手作りが上**・C2 の一致率は幅が広いほど劣化(94.6→85.7)→ **現状 C2 は配給水準でない**(表示は手作り維持が正)。
