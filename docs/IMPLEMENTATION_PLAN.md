...
**次に着手するステップ**: 2026-05-30 #3 並列ワークフロー完了 (**290テスト緑・型0・build/lint緑**)。計画ワークフロー(5候補ブループリント)→ 実装ワークフロー(R4/R24/T2 を互いに素なファイルで並列)→ 中央検証+R11+文書化。今回完了: **R4 概算EV拡張**(`approximate_with_ev` を非BB SRP defender 6 スポットへ・EV coverage 10/21→16/21・新規モデル仮定ゼロ)/ **R24 相手レンジのアクション系列対応**(`opponentRange.ts`・3bet/cold-call 経路を反映・未対応はnull)/ **T2 エラー境界+クラッシュレポート**(`ErrorBoundary.tsx`+`monitoring/reporter.ts`・console既定/Sentry任意・依存追加なし)/ **R11 レンジ%メトリクス整合**(combo比表記+ドリフトガードテスト)。**R16 残ノード(再々レイズ raisesLeft≥2・SBコンプリート)は defer 判断**(エンジン深さ1頭打ち+新規手作りレンジ要=ルール1リスク→R4後)。
**2026-05-30 #4 — R4 honest 3bet/4bet EV ✅**(並列WF: 設計3視点→統合→実装→敵対検証3観点PASS): `attachHeuristicEV.ts` のハードコード `EV(3bet)=0` を実データ駆動(opener応答=`{opener}-vs-{3better}-3bet`)の概算EVに置換し **プリフロップ EV coverage 16/21→21/21**(facing-3bet 5 スポットを `computeOpenerFacing3betEV` で `approximate_with_ev` 化)。fold-equity 項でブラフ3bet>fold を正しく評価・F3=45/F4=60(SRP F=30 不変)。opener応答データ無し 6 spot は ev:0 維持(捏造せず)。全 21 JSON は `approximate_with_ev`(ルール1遵守)。不変条件テスト+9・**300テスト・型0・lint0・build緑**。次は **R4 100BB 真Nash(=サーバ事前計算級・別軸=当面着手不可)/ T2 解析・本番Sentry配線(任意)/ R16残ノード**。先行: R14②/R15/R4-A/R4-B/R25 完了。
