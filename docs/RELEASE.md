...
| **D2 アプリ名 (L4)** | ✅ **「GTO Atlas」へ改名済**(旧「GTO Lab」) | **2026-07-11 決定・実装**。旧名は実在競合「GTO LAB」(gtolab.com)と大小文字を除き完全一致×同一分野で App Store/商標リスク大 → 提出前(可逆)に **GTO Atlas** へ改名(bundle id も `io.github.one-shine.pokergto` へ脱ブランド化)。**残=提出前の専門クリアランス**(あなた): JPO「アトラス」class 9/41(日本先願)+ EUIPO + PokerAtlas 混同確認。"GTO Wizard"/"PokerSnowie"/"GTO LAB" は他社商標→**非提携を明示**(下記 §4 文言)。詳細 [`./BACKLOG.md`](./BACKLOG.md) L4 |
...
### 現状(コード監査)
- ✅ `public/manifest.json`(name=「GTO Atlas — ポーカーGTO学習」/ standalone / icon-192・512 maskable / theme `#18181b`)
...
npx cap init "GTO Atlas" io.github.one-shine.pokergto --web-dir dist
...
```
- 工程B(支払い後): Apple Developer($99/年)→ App ID `io.github.one-shine.pokergto` 登録 → 署名 → §5 メタデータ / §2 年齢区分17+ / §3 "Data Not Collected" / §4 非提携 → Archive → 審査提出。Android も出すなら `npx cap add android`(Google Play $25)。
...
> **GTO Atlas** は独立した学習用アプリです。実際の金銭を賭けるギャンブル機能・換金・賞金は一切ありません(教育・シミュレーション目的)。
> 「GTO Wizard」「PokerSnowie」「GTO LAB」その他のサービスとは**無関係であり、提携・推奨関係はありません**。各名称は各社の商標です。

---

## 5. ストア説明文ドラフト (S4)

**タイトル**: GTO Atlas — ポーカーGTO学習
**サブタイトル**: 実マネー無しのGTO戦略トレーナー(教育用)

**説明(草案・禁止語回避済)**:
> GTO Atlas は、ポーカー(6-max ノーリミットホールデム)の **GTO(ゲーム理論的最適)戦略を学ぶための学習・シミュレーションアプリ**です。実際の金銭の賭けや換金は一切なく、教育・練習を目的としています。
>
...
- [x] ✅ D2: 改名実装済(GTO Lab → **GTO Atlas**・2026-07-11)。**残=提出前の専門クリアランス**(JPO「アトラス」9/41 + EUIPO + PokerAtlas 混同)= 👤(L4)
