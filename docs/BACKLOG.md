...
|----|------|------|------|------------------|
| **R4** | 100BB の open/3bet を真 Nash 解へ | 🧊→🔄 | 🤖 | 現状はプリフロップ全31スポットを `approximate_with_ev`(概算EV)で公開水準に到達。push/fold(≤25BB)のみ厳密解済み。**2026-06-13 再評価**: flop 完全CFRのローカル突破(下行)により、欠落部品だった postflop EVモデルが「実際に解いたサブゲームEV」で構築可能になった。計画(`~/.claude/plans/backlog-a-gto-mac-delegated-candy.md` / `docs/SOLVER.md` 6節): **Phase B**=正準フロップ層化サンプル(N=60)×10ポット構成のサブゲームEVで `(equity−0.5)×30BB` ヒューリスティックを置換 → **Phase C**=モデルゲーム上の fictitious play でプリフロップ均衡を求解(**モデル内Nash**・真の6-max全ゲームNashは16GB RAMで不可のため新sourceティア `solver_model` で正直表示予定)。レンジ置換は判断ゲート付き(C-2a: 解JSONの配給のみ / C-2b: フル置換)。 |
