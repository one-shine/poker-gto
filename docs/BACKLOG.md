...
> **規模感(2026-06-07 精緻化見積もり)**: 残3項目は同じ壁(**カードアブストラクション + ポストフロップEVモデルの不在**)を共有し、依存順序は **flopカードアブストラクション → ポストフロップEVモデル → R4**。規模は R16再々レイズ=**S(1–2日・ブラウザ内・独立・ROI低)** / R16 SBコンプリート=**S–M(2–4日・要レンジ新規作成・R4後が筋)** / flopカードアブストラクション=**L(〜3–6週・オフライン事前計算・A節唯一の現実的な精度向上)** / R4真Nash=**XL(1–数ヶ月・in-browser不可・flop抽象化を内包)**。push/fold(≤25BB)だけが真Nashなのは「全入り=ショーダウン勝率がそのままEV」だから。学習アプリとしては現状の正直表示で公開水準のため **A節着手は任意**。
