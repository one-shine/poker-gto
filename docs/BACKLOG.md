...
| U17 | フォールド後に相手同士のプレイを見続ける必要がない | 🟡 | ⬜ | 自分がフォールドしたら以降の結果に自分は影響せず、AI 同士の決着を遅延付きで見続ける意味が薄い。→ 案: ①フォールド後は残りの AI 送出を**遅延0で一気に決着**させ、結果+New Hand を即出す(`gameStore` のスケジューラで「hero がフォールド済みなら delay=0」分岐 or DealerAgent を即走らせる)/②「次のハンドへ」を即提示。study の答え合わせ(U16・自分のフォールド判断)は**先に見せてから** skip する(学習機会は残す)。設定で「フォールド後すぐ次へ」をトグルできると尚良い。 |
| U18 | GTO戦略だけでなくオッズ基準の望ましいプレイも常に出したい | 🟡 | ⬜ | 現状は GTO戦略(頻度バー)が主で、オッズ基準の目安は**対象外スポット(U16②)と `showPotOdds` の数値表示のみ**。→ **全プレイで GTO戦略に併記**してオッズ基準の推奨(必要勝率 vs 実勝率 → コール/フォールドの目安)を常時出す。`LiveStrategyPanel` のオッズガイド(U16②)を対象外に限らず全スポットへ拡張+提示文を「推奨」化。**要注意(ルール1)**: オッズ基準は純粋なコール判断の単純化で、レイズ/ブラフ/含意オッズ/将来ストリートを含まない=**GTO とは別物**と明示。GTO頻度と食い違う局面(例 オッズ的にはコールだが GTO はミックス/フォールド)での見せ方も要設計。レベル(`showPotOdds`)依存にするか常時かも決める。 |
