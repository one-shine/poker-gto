...
| U36 | モバイル幅(390/360)で日本語が単語途中で改行(コーチ講評ヘッダ「インアキュラ/シー」等) | 🟠 | ✅ | 2026-07-11(ultracode 並列 workflow・実機 Playwright 390 検証): 原因=横並び flex 行が幅超過時に要素ごと縮めて内部でテキスト折返し。対応=**「コンテナは `flex-wrap gap-y-*` で行ごと折返し + 割れNGの短ラベル(セグ/ピル/バッジ/ボタン)は `whitespace-nowrap`」に統一**。`MistakeCard.tsx:80`(ヘッダを flex-wrap 化し重大度ラベル/カテゴリ/EV を個別 nowrap・「次へ」の pr-20 は 44px タップ域確保のため温存しヘッダ折返しで解消)/`DrillQuestion`/`PostflopDrillPanel`(共有 Seg + 盤面/ポット/ストリート群)/`SpotPanel`(各ヘッダバッジ)/`RangesPage`(シナリオ情報行を gap-6→flex-wrap+gap-y)。色/形状/アイコン併用維持。実機 390px で RangesPage・プリフロップ/ポストフロップドリルの各ラベル無傷を目視。CoachPanel は MistakeCard 側で解消したため未編集。627 テスト緑・型0・build緑。 |
| U37 | 学習フィードバック整合2件: 弱点ドリルの深リンクが postflop 系で的外れ出題 / 弱点 TOP3 が全期間累積で克服済みが永久1位 | 🟠 | ✅ | 2026-07-11(ultracode 並列 workflow・敵対的レビューで correctness 確認): ①`preflopDrill.ts` に `POSTFLOP_ONLY_CATEGORIES`(`missed_cbet_ip` 等6種)+述語 `isPreflopDrillCategory` を追加し `scenariosForCategory` が postflop 専用カテゴリで空を返す。従来 `fold_to_3bet`/`call_3bet_oop` が全27シナリオへ無言フォールバックしていたのを **対3bet(`*-3bet`)スポットへ正しくマッピング**。`LearnPage` の DrillTab が深リンクのカテゴリを `isPreflopDrillCategory` で事前振り分けし、postflop 専用の弱点では preflop ドリルを開かず**ポストフロップ・ドリルを初期表示+誘導注記**(無関係出題を出さない保証を上流で担保)。②弱点集計を `MistakeRecord.timestamp` ベースの純関数 `lib/analysis/weaknessWindow.ts`(`aggregateRecentWeaknesses`=直近14日かつ最大100件 / `aggregateAllTimeWeaknesses`)に切り出しテスト。`AnalysisPage` は直近ウィンドウを主+全期間を補助、`LearnPage` ダッシュボードは timestamp 無しの `progress.mistakesByCategory` から timestamped な `sessionStore.mistakes` の直近集計へ切替(見出し「ミス傾向 TOP3(直近14日)」)。新規ストアなし。`weaknessWindow.test`+`preflopDrill.test` 追加。**web(PWA)完結**。 |
| U38 | キャッシュ繰り越しで勝っても自分のキャッシュが増えない(ショーダウンで席スタックが配当未反映) | 🟠 | ✅ | 2026-07-11(ユーザー報告→再現テストで確証): 原因=エンジンは勝者のポット受取を `stackBB` に戻さない設計(`applyAction` は減算のみ・`Showdown` は `amountWonBB` を別途返す)で、`gameStore` の `HAND_COMPLETE` は配当を**次ハンド用 `carryStacks` にしか反映せず、完了時に表示する `gameState.players[hero].stackBB` は拠出後・配当前のまま**だった。reset は翌ハンドで 100 に戻るので無症状だが、cash でスタックを追う人には**ショーダウンで「勝ったのに席の数字が拠出分だけ減る」→「キャッシュが増えない」**体感バグ(再現: 開始81.5→純益+86.5・128.5ポット獲得なのに卓表示39.5)。既存テストは hero が毎ハンド**フォールド=負け経路しか検証しておらず**勝ち経路が盲点。対応=`HAND_COMPLETE` で各席へ受取を加算した `settledState`(=真の終了スタック)を作りポットを空にして表示、cash の持ち越しもこの credited stack を単一ソース化(reset/cash 共通で正しく・チップ保存則維持)。勝ち経路を突く回帰テスト追加(`gameStore.test.ts`)。 |
...
|----|------|------|------|------------------|
| **R4** | 100BB の open/3bet を真 Nash 解へ | 🧊→済 | 🤖 | **A節 GTO 精度=honest 天井で完了(2026-06-13)**。真 Nash は局所不可・GTO近似で運用確定(方針は末尾「A節総括」+ `docs/SPEC.md` A節総括)。push/fold(≤25BB)のみ厳密解済み。**Phase B ✅ 2026-06-13 完了**: `(equity−0.5)×F` ヒューリスティックをサブゲーム解 EV で置換。10 ポット構成×N=60 層化サンプル×約 5 時間(600 ジョブ・6 worker)・全 27 スポット相関 ≥ 0.7(support ゲート後)・AA EV ≈ 3.6BB アンカー確認。id 配線バグ(敵対的レビュー検出)と尾手ノイズ(相関ゲート検出)の 2 バグを修正済み。source は `approximate_with_ev`(戦略は手作りのまま=正直表示)。569 テスト緑・type-check/lint/license 全緑。詳細 `docs/SOLVER.md` § 5。**Phase C 🛑 中止(2026-06-13)**: モデル内 Nash の FP は exploit ≤0.0012 に収束したが、**HU 縮約(opener vs BB)が opener-BB 間のプレイヤーを無視するため早い位置の open が構造的に過広**(UTG 63.5% vs アンカー 13-17% / BTN 57.8% vs 40-50%)。位置依存のオープン幅は HU 縮約では原理的に再現不能で、被覆拡大(外側反復)では直らない → 中止基準を適用しレンジ採用は見送り。真 6-max Nash は multiway 必須で 16GB 不可(本欄当初評価と一致)。求解器コード(`preflopModelGame.ts`/`solve-preflop-nash.ts`)は将来の multiway 化用に保持。`solver_model` ティアは予約のまま未使用。詳細 `docs/SOLVER.md` § 6。<br>**Phase C2-1 ✅ 2026-06-13**: Phase C の構造的限界を「背後プレイヤーを1つのアクション順ゲーム木に入れる」で解決。新規 `preflopMultiwayGame.ts`(6-max 木 + CFR+ + 終端 EV: foldout 厳密 / allin=N-way 厳密 / seen-flop=エクイティ×IP/OOP 非対称実現率・multiway は粗 proxy)+ `preflopEquity.nWayEquity` + `scripts/solve-preflop-multiway.ts`。**木ノード数が C2-0 スパイクと厳密一致**(33,969 決定/5.74M info-set)。致命バグ(終端の reach 正規化で到達確率重みが消失→退化均衡)を `prodOthers` 非正規化重み付けで修正。**結果(600 反復): UTG15.7/MP19.1/CO25.0/BTN41.7 = 4/5 アンカー命中・Phase C の UTG 63.5% → 15.7% = 位置依存オープン幅を構造から回復**・安定性 Δ≤0.4。SB のみ外(no-limp/OOP 実現率の緊張)。全577テスト緑。<br>**Phase C2-2 ✅ 2026-06-13**: ①3bet/4bet サイズを Phase B pot に整合(11/24)②HU seen-flop を Phase B V 行列で評価(UTG/MP/CO/BTN SRP + BTN/CO 3bet・`huSeenFlopEV` 解決器)。**結果(600反復): UTG15.5/MP18.3/CO24.1/BTN40.7 = 4/5 アンカー命中・安定性 Δ≤0.6**。**知見: Phase B V は flat 実現率と差 ≤0.4pt = 較正済 flat が解値をよく近似・open 幅は seen-flop EV 精度に頑健**(「BTN/CO 圧縮=flat 律速」は誤りと実証訂正)。SB のみ据え置き(29.1 vs 35-58 = no-limp 抽象 + OOP 実現率の構造的境界・srp-sb-bb は size/ラベル不整合で未配線=正直表示)。残=**リンプ抽象**(SB 本丸・木拡張)/ 5bet-allin 本求解 / **採用ゲート C-2a(解 JSON 配給)/C-2b(フル置換)= product/正直表示判断・明示承認後**。候補レンジは `scripts/out/`(未採用・gitignore)。詳細 `docs/SOLVER.md §6.5-4/5`。<br>**▶ solver-grade ロードマップ(2026-06-13 計画・公開情報調査済)** = `~/.claude/plans/preflop-solver-grade-roadmap.md`。目標を honest に再定義: **到達可能=デスクトップソルバー水準**(Simple Preflop Holdem/HRC v3 = プリフロップ木+解いた postflop EV・**我々と同系譜**)、**不可=GTO Wizard AI 水準**(NN/クラウド/0.1%pot)。多人数 exploitability は原理的に測れない(業界共通)。診断: C2 は公開 GTO RFI(UTG17.6/MP21.4/CO27.8/BTN43.5)より一貫 **~2-3pt タイト**(静的 flat realization の opener 過小評価=HRC v3 が「静的→postflop 木」で解消した問題と同一)。フェーズ: **V1 ✅**(公開GTOとセル突合: C2 in-out 一致 UTG94.6/MP91.6/CO89.7/BTN85.7%=実シグナルだが MP/CO/BTN は手作り優位で**配給水準でない**。**系統誤差=オフスート高カード過大・スーテッド過小=showdown EV がスーテッドの postflop 実現を報酬化できず → V2 を per-hand データで実証**。`scripts/{published-ranges,validate-preflop}.ts`)→**V2 🛑 中止**(workflow スパイク=GO(HU45×N=20=7.5h)だが、**安価な決定的診断で当初仮説を反証**: ①flat のみ vs Phase B V の per-hand 一致が同一[差≤0.3pt・スーテッド偽陰性も同一]②終端 reach mass の 85.8% が浅SPR 4bet pot[スーテッド≈オフスート]に集中・SRP 7%③解いたモデルは狭い継続レンジしか値を持たず RFI 限界手を含まない → **solved postflop EV は系統誤差を直さない=スーテッド過小は postflop EV 精度でなく抽象の構造問題[行動 funnel]**。中止基準 G2 適用・7.5h の futile compute を回避)→**solver-grade の per-hand 到達は現抽象の天井**。修正は V3 級(複数サイズ/リンプ/flat-call)で payoff 不確実。**A節 GTO 精度はここで honest な天井に到達**=表示は per-hand 優位の手作り維持・solver は構造(位置依存幅)/provenance の研究成果。詳細 `docs/SOLVER.md §6.7`。<br>**▶ V3 計画(ユーザー②選択・payoff 不確実)** = `~/.claude/plans/v3-abstraction-overhaul.md`。行動 funnel 是正でスーテッド忠実度を上げる。**安価レバー実験ファースト**(workflow・worktree 並列)。**V3-0**: A=ハンドクラス realization=**GO**(per-hand +2.7pt・CO/BTN +4.5〜4.9)/ B=3bet 縮小=**NO**(安い 3bet は flat でなく 3bet を増やし SRP 希薄化=仮説の逆)。**V3-1 ✅ 本実装**: realization 理論の `classMult`(連結/ウィール/スーテッドブロードウェイ=1.20・ナッツ性=1.13・disconnected 低 trash=1.05 でテーパー・オフスート=0.90)を seen-flop 終端 EV に配線。**結果: per-hand 一致 UTG95.0/MP93.2/CO95.2/BTN91.7(手作り92.5/94.7/93.5/91.1)= UTG/CO/BTN で手作りを上回り初めて solver が手作り水準に到達**・幅アンカー維持・577テスト緑(classMult 回帰+3)。**正直**: classMult は理論ヒューリスティック(公開RFI 較正)で解いた均衡でない=source は `approximate` 据え置き(solver_model でない)・**solver-grade(解値由来)は現抽象の天井のまま**(V2 確認済)。採用(C-2a/C-2b)は明示承認後。詳細 `docs/SOLVER.md §6.8`。<br>**▶ A節 総括(2026-06-13・方針確定)**: GTO 精度は **honest 天井で完了**。「GTO」と呼べるのは push/fold + 代表フロップのみ・プリフロップは**永久に「GTO近似」**(多人数は商用でも厳密 Nash 不可・exploitability 測れず)。方針: ①正直ラベル固定("GTO最適" 不使用)②**精度の正直開示を UI 機能化**(=方針4・Theory'精度'コンセプト・🔄実装中)③以後プロダクト/配信。**閉じた探索**: V2解いたEV/真マルチウェイNash/リンプ(全て payoff 不可 or ROI低)。solver(C2/V3)は研究成果保持・主表示は手作り維持。詳細 `docs/SPEC.md` A節総括。 |
...
| 収益化方針の決定(iOS限定) | 🔄 | 🤖(整理済)+👤(決定) | **2026-06-14 整理完了(ロードマップ②)= `RELEASE.md` §9「収益化オプション(iOS アプリ限定)」**。方針: **収益化は iOS アプリ限定・PWA は恒久無料/広告なし/プライバシー純粋**。広告は*非ギャンブル(旅行等)*なら内容OK(摩擦=ATT/同意・eCPM・栄養ラベルが収集ありに変化)・**広告収益は Apple 手数料対象外**。IAP は 15-30%。**GitHub Pages 商用 ToS は iOS 収益化に無関係(Cloudflare 移行不要)**。推奨=①無料公開→②非ギャンブル広告+「広告除去」買い切りIAP、または③買い切りPro/コンテンツ販売。サブスクは後回し。競合比較=[`./COMPETITIVE.md`](./COMPETITIVE.md)。**最終決定は 👤**。 |
...
| QR6 | 初回ペイロード削減(スマホ/LTE 体感)| 性能 | ✅ | 2026-07-11(ultracode 並列 workflow・実測ビルドで確認): ①`App.tsx` の `OnboardingFlow`/`ReflectionModal` を既存6ページと同じ `lazy()`+`Suspense` へ(early return は fallback=PageFallback・常時マウントの ReflectionModal は fallback=null)→ **framer-motion の全消費者が lazy 境界の裏に入り初回 index から外れた。index 110.71→66.15kB gzip = -44.56kB(実測・OnboardingFlow 2.77kB / ReflectionModal 1.46kB が別チャンク化)**。②`main.tsx` の和文/latin **500 ウェイト import を削除**(latin は可変フォントが全ウェイト供給=実影響ゼロ・和文 `font-medium` 8箇所は `font-semibold` へ統一しボタン階層維持)。sw プリキャッシュは dist glob で woff2 自動除外。③`fonttools` サブセット CI は**今回スコープ外(defer)**。627 テスト緑・型0・build緑。 |
| QR7 | 障害時のグレースフルデグレード | 堅牢性 | ✅ | 2026-07-11(ultracode 並列 workflow・敵対的レビューで1件是正): ①`sw.js` install の**フォント17件のみ** `Promise.allSettled` の best-effort 個別 fetch+put に分解し1件失敗で起動を止めない。**SHELL(必須シェル)は `cache.addAll` の原子性を維持**(レビュー指摘=SHELL まで best-effort にすると不安定回線初回で `./` 欠落でも install 成功扱い→二度と再試行されずオフライン起動が恒久的に不成立になる為・是正済)。②`solverClient.ts`/`equityClient.ts` に `worker.addEventListener('error')` を追加しモジュール Worker ロード失敗時に**保留中 Promise を全 reject+worker=null 化**(次回はインライン fallback)+防御的タイムアウト(solver120s/equity60s)で永久 pending を排除。型 narrowing 消失(hoisted `cleanup` の `w` possibly null)は非 null const 束縛で是正。③`sound.ts` の resume 条件を `!== 'running'`(iOS 非標準 `interrupted` 包含)へ広げ `visibilitychange`/`pointerdown` でも試行(多重登録ガード付)。jsdom は Worker 無しでテストはインライン fallback を通る=既存テスト緑のまま。**web/PWA 先行**。 |
