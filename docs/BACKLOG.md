...
| U15 | ゲームで「GTO解の対象外」と出る局面でも、レンジは存在するので解を出せるのでは | 🟡 | ✅(整理) | 2026-06-06: **仕様の明確化(コード変更なし)**。対象外になるのは ①マルチウェイ(3人以上=設計ルール4で意図的除外)②未収録のディフェンス(本アプリのプリフロップ厳密収録=各位 RFI + BB 防御5 + 特定ディフェンス6 + 対3bet11 の計27 HUスポット。例 MP vs UTG はレンジ自体が無い)③盲対盲 SRP・4bet 以上の応酬。`resolveSpotKey` の対応範囲は収録データと一致しており、**レンジが存在する clean な HU/RFI スポットは解決する**(取りこぼしバグは無し)。偽の解を出すのはルール1(GTO精度の正直表示)違反のため出さない。残: 未収録ディフェンスのレンジ拡充は R4 で(手作り追加はしない)。`LiveStrategyPanel` の対象外メッセージは理由(マルチウェイ/未収録/盲対盲)を既に明示。 |
